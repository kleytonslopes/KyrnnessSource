...
//}
void FShaderOpenGLComponent::SetVector3(const std::string& name, FVector value)
{
	glUniform3f(glGetUniformLocation(m_ShaderProgramId, name.c_str()), value.X, value.X, value.Z);
}
...
void FShaderOpenGLComponent::SetMatrix4(const std::string& name, FMatrix value)
{
	// Transpor antes de enviar, pois FMatrix é row-major
	float transposed[16];
	for (int row = 0; row < 4; ++row)
		for (int col = 0; col < 4; ++col)
			transposed[col * 4 + row] = value.M[row][col]; // Transposição para column-major

	glUniformMatrix4fv(glGetUniformLocation(m_ShaderProgramId, name.c_str()), 1, GL_FALSE, transposed);
}
