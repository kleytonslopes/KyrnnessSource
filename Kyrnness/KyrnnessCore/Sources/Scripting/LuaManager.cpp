...
void ULuaManager::LoadScript(const std::string& filePath)
{
    try
    {
        ...
        if (mainFunc && mainFunc.value().valid())
        {
            ...
            // Limpa a global "main" pra evitar conflito com o pr�ximo script
            m_LuaState["main"] = sol::nil;
        }
        else
        {
            FLogger::Warning("[Lua] Script %s n�o possui fun��o main().\n", filePath.c_str());
        }
...
void ULuaManager::LoadAllMods()
{
    ...
    for (const auto& modEntry : std::filesystem::directory_iterator(modsBasePath))
    {
        ...
        {
            FLogger::Warning("[Lua] Ignorando Mod: %s (MyMod.lua n�o encontrado)\n", modPath.c_str());
            continue;
        }
...
void ULuaManager::RegisterEngineTypes()
{
    ...
    // Garantir que FVector pode ser chamado como uma fun��o global
...
bool ULuaManager::CallFunction(const std::string& functionName)
{
    ...
    if (!func.valid())
    {
        FLogger::Error("[Lua] Fun��o '%s' n�o encontrada!\n", functionName.c_str());
        return false;
    }
    ...
    {
        sol::error err = result;
        FLogger::Error("[Lua] Erro ao chamar fun��o '%s': %s\n", functionName.c_str(), err.what());
        return false;
    }
