...
static const std::unordered_set<std::string> s_ExcludedExtensions = {
    ".bat",
    ".exe",
    ".dll",
    ".pdb",
...
void UPacker::XOREncrypt(std::vector<uint8_t>& data, uint8_t key)
{
...
void UPacker::PackDirectory_NoEncryption(const fs::path& inputDir, const fs::path& outputFile)
{
    ...
    for (auto& p : fs::recursive_directory_iterator(inputDir))
    {
        ...
        if (p.is_regular_file())
        {
            ...
            if (s_ExcludedExtensions.contains(extension))
            {
                std::cout << "Ignorando arquivo: " << relativePath << " (Extensão: " << extension << ")\n";
                continue;
            }
...
void UPacker::PackDirectory_With_XOR(const fs::path& inputDir, const fs::path& outputFile)
{
    std::vector<FileEntry> entries;
    std::vector<std::vector<uint8_t>> fileDataBlocks;
    uint32_t currentOffset = 0;
    ...
    for (auto& p : fs::recursive_directory_iterator(inputDir))
    {
        ...
        if (p.is_regular_file())
        {
            ...
            XOREncrypt(compressedData, xorKey);
